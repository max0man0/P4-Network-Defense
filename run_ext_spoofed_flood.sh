...
# List of host PIDs based on the formula: base_pid, base_pid+2, base_pid+4, base_pid+6, base_pid+8
host_pids=($(seq $base_pid 2 $((base_pid + 8))))


# Define the path to the python script and the python interpreter
script_path="/home/mez/Protection-Against-ICMPv6/send_ext_syn_spoofed_flood.py"
...
for host_pid in "${host_pids[@]}"; do
    ...
    ipv6_addr=$(mnexec -a $host_pid ip -6 addr show eth0 | grep -oP '(?<=inet6 )2001:[0-9a-fA-F:]+/\d+')
